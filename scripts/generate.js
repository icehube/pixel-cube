  ...
  <body>
      ...
      <div class="control">
        ...
        <details class="multi-select" id="set-filter">
          ...
          <div class="multi-select__controls">
            <input type="text" id="set-filter-search" placeholder="Search sets..." />
            <div class="multi-select__actions">
              <button type="button" id="set-filter-select-all">Select All</button>
              <button type="button" id="set-filter-select-none">Select None</button>
            </div>
          </div>
    ...
    <script>
      ...
      (() => {
        ...
        const setFilterSearchInput = document.getElementById("set-filter-search");
        const setFilterSelectAll = document.getElementById("set-filter-select-all");
        const setFilterSelectNone = document.getElementById("set-filter-select-none");
        ...
        let tierThresholds = null;

        loadData()
          .then((payload) => {
            allCards = Array.isArray(payload.cards) ? payload.cards : [];
            tierThresholds = computeTierThresholds(allCards);
            loadLocalSelection()
              .filter(Boolean)
        ...
        if (setFilterOptionsEl) {
          if (setFilterSearchInput) {
            setFilterSearchInput.addEventListener("input", () => filterSetOptions());
          }
          if (setFilterSelectAll) {
            setFilterSelectAll.addEventListener("click", () => {
              selectAllSets();
            });
          }
          if (setFilterSelectNone) {
            setFilterSelectNone.addEventListener("click", () => {
              clearAllSets();
            });
          }
        }
        ...
        function createCardElement(card) {
          ...
          if (tierLabel) {
            ...
            badge.className = "card__badge " + tierClassName(tierLabel);
...
function matchesRecentFilter(card) {
        ...
        function matchesSetFilter(card) {
          if (!setFilterOptionsEl || !availableSets.size) {
            return true;
          }
          if (!activeSets.size) {
            return false;
        ...
        function populateSetFilters() {
          ...
          entries.forEach(([code, name]) => {
            ...
            label.dataset.search = normalizeSearchText(\`\${name} \${code}\`);
            const checkbox = document.createElement("input");
            checkbox.type = "checkbox";
            checkbox.value = code;
          ...
          updateSetCheckboxes();
          filterSetOptions();
          setFilterOptionsEl.scrollTop = 0;
        }

        function updateSetCheckboxes() {
          if (!setFilterOptionsEl) return;
          setFilterOptionsEl.querySelectorAll('input[type="checkbox"]').forEach((input) => {
            input.checked = activeSets.size ? activeSets.has(input.value) : false;
          });
        }

        function filterSetOptions() {
          if (!setFilterOptionsEl || !setFilterSearchInput) return;
          const term = normalizeSearchText(setFilterSearchInput.value);
          const options = setFilterOptionsEl.querySelectorAll(".multi-select__option");
          options.forEach((label) => {
            ...
            label.hidden = Boolean(term) && !matches;
            label.style.display = matches ? "" : "none";
          });
          setFilterOptionsEl.dataset.filtering = term ? "true" : "false";
        }

        function selectAllSets(options = {}) {
          activeSets.clear();
          availableSets.forEach((_, code) => activeSets.add(code));
          updateSetCheckboxes();
          if (!options.silent) {
        ...
        function areAllSetsSelected() {
          return availableSets.size > 0 && activeSets.size === availableSets.size;
        }
        ...
        function computeTierThresholds(cards) {
          const values = cards
            .map((card) => Number(card.cubeCount) || 0)
            .filter((value) => Number.isFinite(value))
            .sort((a, b) => b - a);
          if (!values.length) {
            return null;
          }
          const q25 = valueAt(values, 0.25);
          const q50 = valueAt(values, 0.5);
          const q75 = valueAt(values, 0.75);
          return { q25, q50, q75 };
        }

        function valueAt(sortedDescValues, fraction) {
          if (!sortedDescValues.length) return 0;
          const idx = Math.min(
            sortedDescValues.length - 1,
            Math.max(0, Math.ceil(sortedDescValues.length * fraction) - 1)
          );
          return sortedDescValues[idx];
        }
        ...
        function getCardTierLabel(card) {
          ...
          const value = Number(card.cubeCount) || 0;
          if (value >= tierThresholds.q25) {
            return "1st";
          }
          if (value >= tierThresholds.q50) {
            return "2nd";
          }
          if (value >= tierThresholds.q75) {
            return "3rd";
          }
          return "4th";
        }

        function tierClassName(label) {
          const key = String(label).trim().charAt(0);
          switch (key) {
            case "1":
              return "card__badge--tier1";
            case "2":
              return "card__badge--tier2";
            case "3":
              return "card__badge--tier3";
            default:
              return "card__badge--tier4";
          }
        }
